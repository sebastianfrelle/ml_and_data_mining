...
from matplotlib.pyplot import (
    figure, hold, plot, title, xlabel, ylabel, colorbar, imshow, xticks, yticks,
    show, suptitle, savefig, close, ylim, cm as colormap, text,
)
...
K_1 = 10
K_2 = 10
OCV = model_selection.KFold(n_splits=K_1, shuffle=False)
ICV = model_selection.KFold(n_splits=K_2, shuffle=False)

# Error matrixs
error_val = np.empty((L, 2, K_2))
error_gen = np.empty((L, 2))
errors = np.zeros(K_1)
errors_weighted = np.zeros(K_1)
...
for par_index, test_index in OCV.split(X_k):
    ...
    for train_index, val_index in ICV.split(X_par):
        ...
        for knn in KNN:
            for d in dist:
                ...
                error_val[int(knn / M - 1), (d - 1), k_2] = np.sum(y_est !=
                                                                   y_val, dtype=float) * (len(X_val) / len(X_par))
    ...
    # Train on X_par
    knclassifier = KNeighborsClassifier(n_neighbors=knn_min_index,
                                        p=d_min_index + 1)
    ...
    test_error_weighted = test_error / len(X_test) * (len(X_test) / len(X_k))
    errors[k_1] = test_error
    errors_weighted[k_1] = test_error_weighted
    ...
    output_path = os.path.join(filepath,
                               f'confusion_matrices/knn/confmat_{k_1 + 1}.png')
    ...
    optimal_parameters.append(
        ParamSet(test_error, knn_min_index, d_min_index + 1))

    k_1 += 1

# Print unweighted, absolute test errors
print('Test errors (Fold <fold_no>: <test_error>):')
for i, t in enumerate(errors, 1):
    print(f'Fold {i}: {t}')


# Print the classification error rate
print('Error rate: {0}%'.format(100 * np.sum(errors_weighted)))
