...
from matplotlib.pyplot import boxplot, figure, show, xlabel, ylabel
...
for train_index, test_index in CV.split(X, y):
    ...
    Error_logreg[k] = 100 * \
        (y_logreg != y_test).sum().astype(float) / len(y_test)
    ...
    Error_dectree[k] = 100 * \
        (y_dectree != y_test).sum().astype(float) / len(y_test)
