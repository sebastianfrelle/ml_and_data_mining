...
from matplotlib.pyplot import (
    figure, plot, title, xlabel, ylabel, savefig, xlim, ylim, margins, bar, xticks,
    show, legend, subplot, suptitle,
)
...
def main():
    ...
    title('Principal component attribute influences')
    bar(range(0, len(V[:, 0])), height=V[:, 0], align='edge')
    xticks(range(0, len(V[:, 0])),
           k_encoded_attr_names + ["" for _ in (range(len(V[0, :]) - len(k_encoded_attr_names)))], rotation=80)
    #    attributeNames + ["" for _ in (range(len(V[0, :]) - len(attributeNames)))], rotation=80)

    savefig('./pc_elements.eps', format='eps', dpi=1000)
    show()
