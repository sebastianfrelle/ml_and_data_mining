...
def rlr_validate(X, y, lambdas, cvf=10):
    ...
    for train_index, test_index in CV.split(X, y):
        ...
        for l in range(0, len(lambdas)):
            ...
            w[:, f, l] = np.linalg.solve(
                XtX + lambdas[l] * np.eye(M), Xty).squeeze()
...
def clusterval(y, clusterid):
    ...
    entropy = ((1 - (p_ij * np.log2(p_ij + EPS)).sum(axis=1))
               * m_i.T).sum() / (N * K)
...
def gausKernelDensity(X, width):
    ...
    log_density = -np.log(N - 1) - M / 2 * \
        np.log(2 * np.pi * width) + np.log(sQ)
